...
struct ExprRes * doArrVal(char * name, struct ExprRes * Res1){
  ...
  //make the string used in the instruction
  char* str1;
  char* str2;
  char* str3;
  str1 = "0(";
...
struct ExprRes * doPow(struct ExprRes * Res1, struct ExprRes * Res2) {
  ...
  //branch to negative case if power is less then 0
  AppendSeq(Res1->Instrs,GenInstr(NULL, "blt",TmpRegName(Res2->Reg),"$zero", neg));//new
  //print the label
  AppendSeq(Res1->Instrs,GenInstr(start, NULL, NULL, NULL, NULL));
  //branch to the end if the counter reaches the power
  AppendSeq(Res1->Instrs,GenInstr(NULL, "beq", TmpRegName(reg), TmpRegName(Res2->Reg), end));
  ...
  //jump to the end
  AppendSeq(Res1->Instrs,GenInstr(NULL, "j", end, NULL, NULL));
  //print the label for the negative case
  AppendSeq(Res1->Instrs,GenInstr(neg, NULL, NULL, NULL, NULL));
  //make the result 0 if the power is negative
  AppendSeq(Res1->Instrs,GenInstr(NULL, "addi", TmpRegName(total), "$zero", "0"));
  //print the ending label
  AppendSeq(Res1->Instrs,GenInstr(end, NULL, NULL, NULL, NULL));
  //relese the registers because we used their result
  ReleaseTmpReg(Res1->Reg);
  ReleaseTmpReg(Res2->Reg);
  ReleaseTmpReg(reg);
...
extern struct InstrSeq * readArr(char * name, struct ExprRes * Res1){
  ...
  inst = (struct InstrSeq *) malloc(sizeof(struct InstrSeq));
  int reg = AvailTmpReg();
...
struct InstrSeq * print(struct ExprResList * ExprList) {
  ...
  while(ExprList != NULL){
    ...
    //if the current attribute is null or not a bool, print a int
    if(ET == NULL || ET->type != "bool"){
    ...
    } else if (ET != NULL && ET->type == "bool"){
      //if here print true or false for  a boolean
	//make the labels
        char * label; 
	char * end;
        label = GenLabel();
...
extern struct InstrSeq * doWhile(struct ExprRes * Res, struct InstrSeq * seq) {
  ...
  AppendSeq(seq2,GenInstr(NULL, "beq", TmpRegName(Res->Reg), "$zero", end));
  //append the body
  AppendSeq(seq2, seq);
  //jump back to the start of the loop
...
extern struct InstrSeq * doFor(struct InstrSeq * seq, struct ExprRes * Res, struct InstrSeq * seq1, struct InstrSeq * seq2) {
  ...
  //branch to the end if the counter reaches the power
  AppendSeq(seq3,GenInstr(NULL, "beq", TmpRegName(Res->Reg), "$zero", end));
  //append the passed in instrs
  AppendSeq(seq3, seq2);
  AppendSeq(seq3, seq1);
  //jump back to the start of the loop
...
struct ExprResList * addElement(struct ExprRes * x, struct ExprResList *h) {
  struct ExprResList * h1;
  h1 = (struct ExprResList *) malloc(sizeof(struct ExprResList));
...
void Finish(struct InstrSeq *Code) {
  ...
  while (hasMore) {
	...
	AppendSeq(code,GenInstr((char *)getCurrentName(table),".word","0",NULL,NULL));
      } else if(ET->type == "array"){
	//if the type is 1 its an array
	char str[100];
	sprintf(str, "%d", ET->size);
